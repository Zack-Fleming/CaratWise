...
The models we will be comparing are K-means, SVM,logistic regression, linear regression, and Random forest.

#### Linear Regression

The first model we decided to test is linear regression.  

```{r}
# Split the data into training and testing sets
sample <- sort(sample(nrow(diamonds_clean), nrow(diamonds_clean) * .70))
...
sample2 <- sort(sample(nrow(diamonds_clean), nrow(diamonds_clean) * .01))
train2 <- diamonds_clean[sample, ] #  of the data set aside for training
test2 <- diamonds_clean[-sample, ] # of the data set aside for testing
```
...
# Tweak the model's features based on what we learned 
<<<<<<< HEAD
linearModel4 <- lm(price ~ carat + clarity + z + color + table, train)
=======
linearModel4 <- lm(price ~ carat  + clarity + z + color + table, train)
>>>>>>> a28f36d0b06134e646bc523cabd722b0ddb93789
...
#the below is for testing purposes 
linearModel5 <- lm(price ~ carat + clarity + z + color + table, train2)
...
# the below is for testing purposes 
diamonds_clean_Linear_Prediction <- predict(linearModel5, test2)

# Output metrics for how the model did
linearModelRsme2 <- mean((test2$price - diamonds_clean_Linear_Prediction)^2) |>
  sqrt()

linearModelRsme2

linearErrorRate2 <- linearModelRsme/mean(test2$price)
linearErrorRate2
```
...
The second model we are comparing is Support Vector Machine, or SVM. This model is also a good candidate for regression predictions.
...
With the model created, we can make the predictions and caclulate the relative accuracy of the model.

```{r}
svm_predict <- predict(svm1, newdata = test)
svm_table <- table(test$price, svm_predict)
...
#### Random Forest or Decison Tree

```{r}
rf <- randomForest(
  price ~ carat  + clarity + z + color + table, data=train
)
rf
```

```{r}
rf_predict_claim = predict(rf, newdata = test, type = "class")
rf_table <- table(test$price, predict_claim)
