...
sample <- sort(sample(nrow(diamonds_clean), nrow(diamonds_clean) * .75))
train <- diamonds_clean[sample, ] # 75% of the data set aside for training
test <- diamonds_clean[-sample, ] # 25% of the data set aside for testing
```
...
linearModel2 <- lm(price ~ carat + cut + colorless + clarity + table, train)
...
# Tweak the model's features based on what we learned 
linearModel3 <- lm(price ~ carat + cut + colorless, train)
