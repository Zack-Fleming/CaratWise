...
The summary shows that the dataset at least has no missing values, and that that some columns may have some outliers or erroneous data. 
...
In an effort to make this correlation matrix more readable, we can turn it into a heatmap. A heatmap is helpful, since it uses a different color negative and positive correlation. Also, the heatmap takes out the redundant info from the correlation matrix. We can also take the heatmap and make decisions of what columns to use and discard for the prediction models.
...
plot1 <- ggplot(data = diamonds_raw, aes(x=carat)) +
  ...
  geom_vline(aes(xintercept = mean(carat)), color = "blue", linetype="dashed", linewidth = 1)
...
grid.arrange(plot1, plot2, ncol=2) # arrange the plots together
```
...
plot1 <- ggplot(data = diamonds_raw, aes(x=depth)) +
  geom_histogram(color = "black", fill = "white") + 
  geom_vline(aes(xintercept = mean(depth)), color = "blue", linetype="dashed", linewidth = 1)
...
plot3 <- ggplot(data = diamonds_raw, aes(x=table)) +
  geom_histogram(color = "black", fill = "white") + 
  geom_vline(aes(xintercept = mean(table)), color = "blue", linetype="dashed", linewidth = 1)
...
grid.arrange(plot1, plot2, plot3, plot4, ncol=2, nrow = 2) # arrange the plots together
```
...
plot3 <- ggplot(data = diamonds_raw, aes(x=y)) +
  geom_histogram(color = "black", fill = "white") + 
  geom_vline(aes(xintercept = mean(y)), color = "blue", linetype="dashed", linewidth = 1)
...
plot5 <- ggplot(data = diamonds_raw, aes(x=z)) +
  geom_histogram(color = "black", fill = "white") + 
  geom_vline(aes(xintercept = mean(z)), color = "blue", linetype="dashed", linewidth = 1)
...
Summary of data before handling outliers.
...
summary(diamond_outliers1$carat)
```

We lost 439 records, which represents 0.81% of the data. 
...
summary(diamond_outliers2$depth)
```

Now we lost 663 more records (or a total of 1102 records), which represents 2.04% of the full dataset.
...
Another 264 records are lost (1366 cumulative), which represents a cumulative loss of 2.53% of the dataset.
...
Six more records lost (1372 cumulative), representing a total loss of 2.54% of the dataset.
...
Another two records was lost (1374 cumulative), representing a total loss of 2.547% of the dataset.
...
diamonds_clean <- diamond_outliers5 |>
  mutate(zscore = (z - the_mean)/the_sd) |>
  filter(zscore <= 3 & zscore >= -3)


summary(diamonds_clean$z) #Lost 14 records which is a cumulative percentage of 2.60% of the total dataset and 0.03% percent of the outlier2 data set
```

Fourteen more records lost (1388 cumulative), which represents a final loss of 2.6% of the data. This percentage of data lost is negatable.


## Encoding Categorical Columns

We will now re encode the categorical columns into a more logical/numeric columns, for use in some of the models going forward. We will be encoding the following columns in the following way:

 1. color goes to Colorless: D-F will be colorless (1) and G-J will be colored/tinted (0)
 2. cut will be pivot longer, converting it into a set of 'logical' columns
 3. clarity will also be pivot longer, converting it into a set of 'logical' columns

```{r}
# converting the color to 'logical'
diamonds_clean <- diamonds_clean |>
  mutate(colorless = ifelse(color == "D" | color == "E" | color == "F", 1, 0))

# converting the cut to 'logical'
diamonds_clean <- diamonds_clean |>
  mutate(cut.Ideal = ifelse(cut == "Ideal", 1, 0)) |>
  mutate(cut.Premium = ifelse(cut == "Premium", 1, 0)) |>
  mutate(cut.Very_Good = ifelse(cut == "Very Good", 1, 0)) |>
  mutate(cut.Good = ifelse(cut == "Good", 1, 0)) |>
  mutate(cut.Fair = ifelse(cut == "Fair", 1, 0))

# converting the clarity to 'logical'
diamonds_clean <- diamonds_clean |>
  mutate(clarity.IF = ifelse(clarity == "IF", 1, 0)) |>
  mutate(clarity.VVS1 = ifelse(clarity == "VVS1", 1, 0)) |>
  mutate(clarity.VVS2 = ifelse(clarity == "VVS2", 1, 0)) |>
  mutate(clarity.VS1 = ifelse(clarity == "VS1", 1, 0)) |>
  mutate(clarity.VS2 = ifelse(clarity == "VS2", 1, 0)) |>
  mutate(clarity.SI1 = ifelse(clarity == "SI1", 1, 0)) |>
  mutate(clarity.SI2 = ifelse(clarity == "SI2", 1, 0)) |>
  mutate(clarity.I1 = ifelse(clarity == "I1", 1, 0))
```

Now that the data wrangling is done, as far as we know, we can finally get into the training of the prediction models.

## Linear Model Predictions

The model we decided on using was the linear regression model. This model is fairly simple and easy to use.   

```{r}
# Split the data into training and testing sets
set.seed(1)
sample <- sort(sample(nrow(diamonds_clean), nrow(diamonds_clean) * .80))
train <- diamonds_clean[sample, ] # 70% of the data set aside for training
test <- diamonds_clean[-sample, ] # 30% of the data set aside for testing

```

Next we begin to train the model.

```{r}
# Initial model 
linearModel1 <- lm(price ~ carat + cut + colorless + clarity + depth + table + x + y + z, train)

summary(linearModel1)

# check for correlations among independent variables
cor(diamonds_clean[, c(1,5,6, 8:10, 12:25)])
```
Based off of the initial training, we make changes to the model. 

```{r}
# Tweak the model's features based on what we learned 
linearModel2 <- lm(price ~ carat + cut + colorless + clarity + table + z, train)

# Check how the new model compares 
summary(linearModel2)

# Recheck correlations 
cor(diamonds_clean[, c(1,5,6, 12:25)]) 


# Tweak the model's features based on what we learned 
linearModel3 <- lm(price ~ carat + cut + clarity, train)
...
# Tweak the model's features based on what we learned 
linearModel4 <- lm(price ~ carat  + clarity + color + table, train)
...
The last step in linear regression is to evaluate the model using the test data that we set aside earlier. 

```{r}
# Pass the test data into the model and run
diamonds_clean_Linear_Prediction <- predict(linearModel4, test)

# Output metrics for how the model did
linearModelRsme <- mean((test$price - diamonds_clean_Linear_Prediction)^2) |>
  sqrt()

linearModelRsme

linearErrorRate <- linearModelRsme/mean(test$price)
linearErrorRate
...
The final model that we decided to use for testing was linearModel4.  This model resulted in approximately 71% accuracy in testing.  Predictions of diamond prices are within approximately +-$1,100. Based on the features in the dataset, this is as accurate a model as possible.  One of the things that we learned during our analysis is that even with a substantial amount of data, it is challenging to correctly predict the price of diamonds with a high degree of accuracy. Also, if we were predicting something else, like carat, the model may have performed better; as when researching this dataset, most sources had the goal of predicting the carat of the diamond, not price.


## Breakdown of Contributions

Zack: Visualizations, data wrangling, modeling

Cristian: Data wrangling, modeling 

Jake: Data wrangling, modeling 
